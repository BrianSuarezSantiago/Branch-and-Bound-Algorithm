...
def argmin_list(seq, fn):
    ...
    >>> argmin_list(['one', 'two', 'three', 'or'], len)
    ['two', 'or']
    """
...
# Performs different operations that change the state of the graph
orientations = [(1, 0), (0, 1), (-1, 0), (0, -1)]

def turn_right(orientation):
    """ Performs a right turn as indicated by the argument orientation. """
    return orientations[orientations.index(orientation) - 1]

def turn_left(orientation):
    """ Performs a left turn as indicated by the argument orientation. """
...
def clip(vector, lowest, highest):
    ...
    value of lowest or more than the corresponding value of highest, clip
    to those values.
    >>> clip((-1, 10), (0, 0), (9, 9))
    (0, 9)
    """
...
def if_(test, result, alternative):
    ...
    both result and alternative are always evaluated. However,
    if either evaluates to a function, it is applied to the
    empty arglist, so you can delay execution by putting it in
    a lambda.
...
class Queue:
    ...
    def extend(self, items):
        """ Adds values to the data structure. """
        for item in items:
            self.append(item)
...
class BranchBound(Queue):
    """
    Class that implements the Branch and Bound algorithm using
    as a sorting function the sum of the costs of each of the
    paths between two specified nodes.
    """
...
class BranchBoundUnderestimation(Queue):
    """
    Class that implements the Branch and Bound algorithm using
    as a sorting function the sum of the costs of each of the
    paths between two specified nodes.

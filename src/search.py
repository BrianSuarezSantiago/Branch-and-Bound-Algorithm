...
class Problem:
    ...
    path_cost. Then you will create instances of your subclass and solve
    them with the various search functions.
    """
...
class Node:
    """
    A node in a search tree. Contains a pointer to the parent (the node that
    this is a successor of) and to the actual state for this node. Note that
    if a state is arrived at by two paths, then there are two nodes with the
    same state.  Also includes the action that got us to this state, and the
...
def graph_search(problem, fringe):
    """
    Search through the successors of a problem to find a goal. The
    argument fringe should be an empty queue. If two paths reach a
    state, only use the best one.
    """
    ...
    while fringe:
        ...
        if problem.goal_test(node.state):
            # Creates the formatted string to be displayed in the output
            # of the execution of the corresponding implemented algorithms.
            print("Expanded nodes: " + f'{expanded_nodes_count}'
                + "\nVisited nodes: " + f'{visited_nodes_count}' 
...
# Invocations of the different search methods implemented
def breadth_first_graph_search(problem):
    """ Search the shallowest nodes in the search tree first. """
    return graph_search(problem, FIFOQueue())    # FIFOQueue --> fringe (open list manager)
...
def branch_and_bound(problem):
    ...
    return graph_search(problem, BranchBound())    # Branch & Bound --> fringe

def branch_and_bound_with_underestimation(problem):
    """ Branch and bound with underestimation algorithm implementation. """ 
    return graph_search(problem, BranchBoundUnderestimation(problem))    # Branch & Bound with Underestimation --> fringe
